...
def load_model_config(checkpoint_path, target_id):
...
def load_training_checkpoint(path, model, optimizer):
    log("Resuming training from V2 checkpoint")
...
def main():
    log("Starting OCR training pipeline V2")
    ...
    dataset_paths = [
        ...
        "../data-collection/collected-data-synthetic-v2"
    ]

    model_progress_checkpoint = "./checkpoint/model_progress_checkpoint.json"
    model_checkpoint_dir = "./checkpoint/model"

    best_model_path = os.path.join(model_checkpoint_dir, "model_0014_best.pt")
    resume_path = "./checkpoint/training_resume_v2.pt"
    ...
    if os.path.exists(resume_path):
        ...
        log("No V2 resume checkpoint found")
        log("Loading BEST model as initialization")
        model.load_state_dict(torch.load(best_model_path, map_location=device))

    # ---------------- MLflow ----------------
    mlflow.set_experiment("crnn_ocr_finetune")
    mlflow.start_run(run_name="model_0014_v2")

    mlflow.log_params(configuration)
    mlflow.set_tag("phase", "finetune_v2")
    ...
    try:
        ...
        for epoch in range(start_epoch, configuration["training"]["epochs"] + 1):
            ...
            log(f"Epoch {epoch} finished | Train {train_loss:.4f} | Val {val_loss:.4f}")
